...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                               ...
                               QComboBox, QSpinBox, QDoubleSpinBox, QTextEdit, QFileDialog, QProgressBar)
...
class NASApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MICRONAS ENGINE v5")
        ...
        # self.signals.plot.connect(self.update_plot)
    ...
    def init_ui(self):
        ...
        self.hw_label = QLabel(f"Hardware: {hw_text}")
        ...
        # Config params
        left_layout.addWidget(QLabel("Population Size:"))
        ...
        # Start button
        self.btn_start = QPushButton("Start NAS")
        self.btn_start.clicked.connect(self.start_nas)
        left_layout.addWidget(self.btn_start)

        left_layout.addStretch()

        # Right Panel - Logs and Viz
        right_panel = QWidget()
        ...
        # Plot label (placeholder)
        self.lbl_plot = QLabel("Pareto Frontier Plot Will Appear Here")
        self.lbl_plot.setAlignment(Qt.AlignCenter)
        right_layout.addWidget(self.lbl_plot)
    ...
    def select_dataset(self):
        # Allow choosing a directory (for images) or file (for tabular)
    ...
    def start_nas(self):
        ...
        self.log_message("Starting NAS Pipeline...")
    ...
    def run_nas_process(self):
        try:
            ...
            self.signals.progress.emit(f"Data loaded. Input Shape: {input_shape}, Classes: {num_classes}")
            ...
            from micro_nas_engine.visualization.plots import plot_pareto_frontier
            plot_b64 = plot_pareto_frontier(engine.history)
            if plot_b64:
                self.signals.plot.emit(plot_b64)

            self.signals.progress.emit("Starting final training on the best architecture...")
            best_arch = results[0]
    ...
    def on_nas_finished(self, results):
        self.progress_bar.setVisible(False)
        self.btn_start.setEnabled(True)
        if results:
            best = results[0]
            self.log_message(f"NAS Finished! Best Acc: {best['accuracy']:.2f}%, Cost: {best['cost']}")
        else:
